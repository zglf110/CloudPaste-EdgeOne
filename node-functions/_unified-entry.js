  ...
  dbInitPromise = (async () => {
    ...
    if (platform === "edgeone") {
      ...
      try {
        ...
        logger.info("MySQL 数据库连接成功，EdgeOne Pages 环境已就绪");
        return dbAdapter;
      } catch (error) {
    ...
    const cfLogger = createLogger("Cloudflare/Init", env);
    if (!env?.DB) {
      cfLogger.error("DB 未绑定");
      throw new Error("DB 未绑定，请在 Cloudflare 绑定中配置 D1 数据库");
    }

    cfLogger.info("使用 Cloudflare D1 数据库");
    dbAdapter = env.DB;
    await ensureDatabaseReady({ db: dbAdapter, env });
    cfLogger.info("Cloudflare Workers 环境已就绪");
    return dbAdapter;
  })();
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      if (!env || !env.ENCRYPTION_SECRET) {
        logger.error("ENCRYPTION_SECRET 未配置");
  ...
  async scheduled(controller, env, ctx) {
    const logger = createLogger("Scheduled", env);
    ...
    try {
      ...
      if (platform === "edgeone") {
        logger.warn("EdgeOne Pages 环境暂不支持 scheduled 触发器，跳过维护任务执行");
        return;
      }

      if (!env || !env.DB) {
        logger.warn("缺少 DB 绑定，跳过维护任务执行");
        return;
      }

      logger.info("Cloudflare scheduled 触发，开始检查到期后台任务", {
      ...
      const logger = createLogger("Scheduled", env);
      logger.error("执行维护任务时发生错误", error);
    }
  },
};
